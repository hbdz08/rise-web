...
type ImportPreview = {
  ...
  failCount: number;
  errors: Array<{ row: number; column?: string; message: string }>;
  rows: Array<{ row: number; name: string; dept: string; jobTitle: string | null; status: "active" | "inactive"; op: "create" | "update" }>;
};
      ...
      <PageHeader
        ...
        actions={
          <Space>
            ...
            <Upload
              accept=".xlsx"
              action="/api/admin/employees/import?preview=1"
              withCredentials
            ...
            >
              <Button type="tertiary">预校验</Button>
            </Upload>
            <Upload
              accept=".xlsx"
        ...
        footer={
          <Space>
            ...
            <Button
              ...
              disabled={!importFile || !importPreview || importPreview.failCount > 0}
              onClick={() => {
                if (!importFile) {
                  Toast.error("请先选择文件");
                  return;
                ...
                void (async () => {
                  ...
                  try {
                    const json = await postXlsx("/api/admin/employees/import", importFile);
                    if (!json?.ok) {
                      Toast.error(json?.message ?? "导入失败");
                      return;
                    }
          ...
          <Space vertical style={{ width: "100%" }} spacing="medium">
            ...
            <Table
              ...
              columns={[
                ...
                { title: "部门", dataIndex: "dept", width: 180 },
                { title: "职位", dataIndex: "jobTitle", width: 180 },
                {
                  title: "状态",
                  dataIndex: "status",
                  width: 120,
                  render: (v: string) => (v === "active" ? <Tag color="green">启用</Tag> : <Tag color="grey">停用</Tag>),
                },
              ]}
            ...
            />
            <Text type="tertiary">预览最多展示 100 行。</Text>
          </Space>
        )}
      </Modal>
    </Space>
