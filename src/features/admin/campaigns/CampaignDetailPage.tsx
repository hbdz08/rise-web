...
import { Button, Card, Descriptions, Form, Modal, Space, Tag, Toast } from "@douyinfe/semi-ui-19";
...
export default function CampaignDetailPage() {
  ...
  const canAdmin = role === "HR_ADMIN";
  const canPublish = canAdmin && campaign?.status === "draft";
  const canArchive = canAdmin && campaign?.status === "published";
          ...
          <Space>
            ...
            <Button
              ...
              onClick={() => {
                ...
                Modal.confirm({
                  title: "确认发布？",
                  content: "发布后普通 HR 不允许再修改金额/绩效；仅管理员可修正（需填写原因>=5字）。",
                  onOk: async () => {
